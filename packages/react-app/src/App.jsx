  ...
  for (let a in loadedAssets) {
    ...
    if (loadedAssets[a].forSale) {
      ...
      if (loadedAssets[a].owner == address && !loadedAssets[a].forSecondarySale) {
        cardActions.push(
          <Button onClick={() => {
            tx(writeContracts.MAKERGENESIS.sellItem(loadedAssets[a].id, { gasPrice: gasPrice }))
          }}>
            Sell
          </Button>
        )
        ...
        cardActions.push(
          <Button onClick={() => {
            tx(writeContracts.MAKERGENESIS.cancelSellItem(loadedAssets[a].id, { gasPrice: gasPrice }))
          }}>
            Cancell Sell
          </Button>
        )
  ...
  return (
    <div className="App">
      ...
      <div className="ringView">
        <model-viewer bounds="tight" src="/model.glb" ar ar-modes="webxr scene-viewer quick-look" camera-controls environment-image="neutral" poster="poster.webp" shadow-intensity="3.7" exposure="2" shadow-softness="0.39">
      ...
      <BrowserRouter>
        ...
        <Menu style={{ textAlign: "center" }} selectedKeys={[route]} mode="horizontal">
          ...
          <Menu.Item key="/yourcollectibles">
            <Link onClick={() => { setRoute("/yourcollectibles") }} to="/yourcollectibles">MAKERGENESIS</Link>
          </Menu.Item>
                    ...
                    <List.Item key={id + "_" + item.uri + "_" + item.owner}>
                      ...
                      <div>
                        ...
                        <Button onClick={() => {
                          ...
                          tx(writeContracts.MAKERGENESIS.transferFrom(address, transferToAddresses[id], id))
                        }}>
                          Transfer
                        </Button>
                      </div>
